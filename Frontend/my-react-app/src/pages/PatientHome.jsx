...
export default function Home({ darkMode, searchQuery, setSearchQuery, setCurrentPage, setSelectedDoctor, setBookingData }) {
  const { user, isLogin } = useContext(AuthContext); // Use your context values
  const [doctors, setDoctors] = useState(fallbackDoctors);
  
  // Get name from Context OR LocalStorage as fallback
  const displayName = user?.name || localStorage.getItem('userName') || 'User';
  const isLoggedIn = isLogin || !!localStorage.getItem('token');

  // --- FETCH DOCTORS FROM API ---
  useEffect(() => {
    const fetchDoctors = async () => {
      try {
        // Get user location from storage to filter search
        const userLoc = JSON.parse(localStorage.getItem('userLocation'));
        const city = userLoc ? userLoc.city : '';
        ...
        if(response.data.results && response.data.results.length > 0) {
            ...
            // If API returns empty (no matches), you might want to show empty or fallback
            // setDoctors([]); 
        }
      } catch (error) {
        console.error("Failed to fetch doctors:", error);
        // Keep fallbackDoctors if error
      }
    };

    // Debounce: Only search if query exists or on load
                ...
                <div className="p-6">
                  <div className="flex items-start gap-4 mb-4">
                    <div className="relative">
                      <div className="text-6xl transform group-hover:scale-110 transition-transform">{doctor.photo || "👨‍⚕️"}</div>
                  ...
                  <div className="flex gap-2">
                    <button 
                      onClick={() => { 
                        ...
                        setBookingData(prev => ({ ...prev, slot: doctor.slots ? doctor.slots[0] : '10:00 AM' })); 
