...
import { Zap, Search, Shield, Video, Star, MapPin, Clock, MessageCircle } from 'lucide-react';
      ...
      <div className="container mx-auto px-4 py-8">
          ...
          <div className="relative z-10 max-w-2xl">
            ...
            <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-2 flex flex-col md:flex-row gap-2 border border-white/20">
              <div className="flex-1 flex items-center gap-3 px-4">
                ...
                <input type="text" placeholder="Search doctors, specialties, clinics..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="flex-1 py-4 bg-transparent outline-none placeholder-white/70" />
              </div>
              <button onClick={() => setCurrentPage('search')} className="bg-white text-blue-600 px-8 py-4 rounded-xl hover:shadow-lg transition-all font-semibold flex items-center justify-center gap-2">
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-12">
          {[
            ...
            <div key={idx} className={`p-6 rounded-2xl backdrop-blur-lg border ${darkMode ? 'bg-gray-800/50 border-gray-700' : 'bg-white/50 border-white'} text-center group hover:scale-105 transition-transform`}>
        ...
        <div className="mb-12">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {specialties.map((spec, idx) => (
              <div key={idx} onClick={() => { setSearchQuery(spec.name); setCurrentPage('search'); }} className={`p-4 rounded-2xl text-center cursor-pointer group backdrop-blur-lg border ${darkMode ? 'bg-gray-800/50 border-gray-700 hover:bg-gray-700/50' : 'bg-white/50 border-white hover:bg-white'} transition-all hover:scale-105 hover:shadow-lg`}>
        ...
        <div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {mockDoctors.map(doctor => (
              <div key={doctor.id} className={`rounded-2xl overflow-hidden group backdrop-blur-lg border ${darkMode ? 'bg-gray-800/50 border-gray-700' : 'bg-white/50 border-white'} hover:shadow-xl transition-all duration-300`}>
                <div className="p-6">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => { setSelectedDoctor(doctor); setBookingData(prev => ({ ...prev, slot: doctor.slots[0] })); setCurrentPage('profile'); }} className="flex-1 bg-gradient-to-r from-blue-600 to-purple-600 text-white py-3 rounded-xl hover:shadow-lg transition-all font-semibold">Book Now</button>
                    <button onClick={() => { setSelectedDoctor(doctor); setCurrentPage('chat'); }} className="p-3 border border-gray-300 dark:border-gray-600 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"><MessageCircle size={20} /></button>
                  </div>
                </div>
              </div>
            ))}
